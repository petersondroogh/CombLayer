...
namespace essSystem
{
  ...
  FlightLine::FlightLine(const std::string& Key)  :
    attachSystem::ContainedGroup("inner","outer", "liner", "linerAl"),
  ...
  FlightLine::FlightLine(const FlightLine& A) : 
    ...
    plateIndex(A.plateIndex),nLayer(A.nLayer),lThick(A.lThick), lHeight(A.lHeight),
  ...
  */
  {
	...
	lHeight.push_back(Control.EvalVar<double>(keyName+"LinerHeight"+idxStr));
  ...
  */
  {
    ...
    ModelSupport::buildPlane(SMap,flightIndex+3,Origin-X*(width/2.0-0),xDircA); // inner bottom (y<0) plane inside Be
    ModelSupport::buildPlane(SMap,flightIndex+4,Origin+X*(width/2.0-0),xDircB); // inner upper (y>0) plane inside Be
	...
	ModelSupport::buildPlane(SMap,flightIndex+II*10+13, Origin-X*(width/2.0)-xDircA*layT,xDircA);
	ModelSupport::buildPlane(SMap,flightIndex+II*10+14, Origin+X*(width/2.0)+xDircB*layT,xDircB);
	ModelSupport::buildPlane(SMap,flightIndex+II*10+15, Origin-Z*(height/2.0)-zDircA*layH,zDircA);
	ModelSupport::buildPlane(SMap,flightIndex+II*10+16, Origin+Z*(height/2.0)+zDircB*layH,zDircB);
      }
	...
	//	std::cout << "here sNum: " << i << " " << SMap.realSurf(sNum) << std::endl;
	FixedComp::setLinkSurf(i-1,signVal*SMap.realSurf(sNum));
	const int tNum(flightIndex+static_cast<int>(i));
	//	std::cout << "here tNum: " << i << " " << SMap.realSurf(tNum) << std::endl;
	...
	if (capLayer[i])
	  {
		...
		capThickStack.push(capThick+lThick[i]); // kbat: when shall we use lHeight?
	      }
	    else
	      {
		capThickStack.pop();
  ...
  */
  {
    ...
    Out+=attachRule;         // forward boundary of object (inner radius of Be cylinder)
    ...
    Out=ModelSupport::getComposite(SMap,flightIndex," 3 -4 5 -6 ");
    Out+=attachRule;         // forward boundary of objetct (inner radius of Be cylinder)
    Out+=" "+ContainedGroup::getContainer("outer");      // Be outer surface
    System.addCell(MonteCarlo::Qhull(cellIndex++,0, 0.0,Out)); // void instead of Be - set He material here
	...
	Out1=Out;
	Out1+=" "+ContainedGroup::getContainer("liner");
	System.addCell(MonteCarlo::Qhull(cellIndex++,lMat[i],0.0,Out1));
	// !!! this breakes the TDR model - understand how capping works (Stuart does not remember what is capping) !!!
	Out1=Out;
	Out1+=" "+ContainedGroup::getContainer("outer");      // Be outer surface
	HeadRule HR;
	HR.procString(ContainedGroup::getContainer("linerAl"));
	HR.makeComplement();
	Out1 += " " + HR.display();
	//	Out1+=" "+ModelSupport::getComposite(SMap, abs(atoi(ContainedGroup::getContainer("linerAl").c_str()))-1, "1");
	System.addCell(MonteCarlo::Qhull(cellIndex++,0,0.0,Out1));

	Out1=Out;
	Out1+=" "+ContainedGroup::getContainer("linerAl");
	HR.procString(ContainedGroup::getContainer("liner"));
	HR.makeComplement();
	Out1 += " " + HR.display();
	//	ELog::EM << HR.display() << ELog::endDiag;
	//	Out1+=" "+ModelSupport::getComposite(SMap, abs(atoi(ContainedGroup::getContainer("liner").c_str()))-1, "1");
	System.addCell(MonteCarlo::Qhull(cellIndex++,lMat[0],0.0,Out1));
