...
BulkModule::createObjects(Simulation& System,
			  const attachSystem::ContainedComp& CC1,
			  const attachSystem::ContainedComp& CC2)
  /*!
  ...
  */
{
  ...
  for(size_t i=0;i<nLayer;i++)
	...
	OutX=CC1.getExclude() + CC2.getExclude(); 
...
BulkModule::addFlightUnit(Simulation& System,
  ...
  /*!
    ...
    \param REF is used to exclude BeRef (top or bottom)
  */
{
  ...
  std::stringstream cx;
  cx<<" (";
  for(size_t index=2;index<6;index++) { // changed index<5  -> index<6 in order to move liners inside Be
    cx<<FC.getLinkString(index+6)<<" : "; // to move liners inside Be
  }
  ...
  for(size_t index=2;index<6;index++) {
    cx<<FC.getLinkComplement(index+6)<<" "; // to move liners inside Be
  }

  cx << " " << REF.getLinkString(0) << " ";

  Out=ModelSupport::getComposite(SMap,bulkIndex+(static_cast<int>(nLayer)-1)*10, " -7  "); // to accomodate number of layers (17 with 2 layers, 27 with 3 layers)
  ...
  for(int i=1;i<static_cast<int>(nLayer);i++)
    {
      MonteCarlo::Qhull* OPtr=System.findQhull(bulkIndex+i+1); // +1
      if (!OPtr)
	//	throw ColErr::InContainerError<int>(bulkIndex+i,"layerCells");
      throw ColErr::InContainerError<int>(bulkIndex+i+1,"layerCells");
      OPtr->addSurfString(cx.str());
    }
  // Now make internal surface
...
void BulkModule::addToInsertChain(attachSystem::ContainedComp& CC) const
{
