...
namespace essSystem
{
  ...
  */
  {
	...
	// kbat: Gap between target and premoderator:
	if (i==nLayers-1) { 
	  ModelSupport::buildCylinder(SMap, SI+9, Origin, Z, radius[i]-(depth[i]-depth[i-1])); // for radial Al container, to avoid additional variables we set its thickness to (depth[i]-depth[i-1]) = TopPreDepth3
	}
  ...
  /*!
    ...
    \param Target :: Target wheel
    \param rIndex :: Target wheel upper/bottom surface link for TopPre/LowPre
  */
  {
    ELog::RegMethod RegA("CylPreMod","createObjects");

    const std::string RSurf=(rIndex<0) ? Target.getLinkComplement(static_cast<size_t>(-(rIndex+1))) : Target.getLinkString(static_cast<size_t>(rIndex));
    ELog::EM << "rIndex: " << rIndex << ELog::endDiag;
    ELog::EM << "RSurf: " << RSurf << ELog::endDiag;

    std::string Out, strtmp;
    layerCells.clear();
	...
	if (i!=nLayers-1)
	  ...
	  // kbat: Gap between target and premoderator
	  Out=ModelSupport::getComposite(SMap,SI, " -9 -5 ") + RSurf; // void between upper target plane and water.
	  System.addCell(MonteCarlo::Qhull(cellIndex++, 0, 0.0, Out));
	  

	  // Al container - radial direction
	  //	  std::cout << "CylPreMod::createObjects()\t FIX THIS AWFULL HACK" << std::endl;
	  //	  Out = ModelSupport::getComposite(SMap, SI, atoi(RSurf.c_str())-1, "(-7 1M -5) (9:-1M:5)"); // will not work if RSurfPlus is not integer !!! todo !!!
	  //	  if (atoi(RSurf.c_str())>0) // top tier
	  ///	  Out = ModelSupport::getComposite(SMap, SI, atoi(RSurf.c_str())-1, "(-7 1M -5) 9"); // will not work if RSurfPlus is not integer !!! todo !!!
	  //	  Out = ModelSupport::getComposite(SMap, SI, atoi(RSurf.c_str())-1, "-7 1M -5 9"); // will not work if RSurfPlus is not integer !!! todo !!!
	  Out = ModelSupport::getComposite(SMap, SI, "-7 -5 9"); // will not work if RSurfPlus is not integer !!! todo !!!
	  Out+=RSurf;//.c_str();

	  // The way to make complement to the surface:
	  
	  /*
	  HeadRule HR;
	  HR.procString(RSurf); //.c_str()); 
	  HR.makeComplement();
	  std::cout << "AAA " << RSurf << " compl " << HR.display() << std::endl;
	  //	  Out += HR.display();
	  */

	  //Out=ModelSupport::getComposite(SMap,SI, strtmp); // Al container of the gap in the radial direction
	  System.addCell(MonteCarlo::Qhull(cellIndex++, mat[i], temp[i], Out)); // to avoid additional variable we set the same material as Material3

	  Out=ModelSupport::getComposite(SMap,SI," -7  -6 ");
	  Out += RSurf;
	  
	  addOuterSurf("Main",Out);
	  Out=ModelSupport::getComposite(SMap,SI," -7 5 -6 "); // kbat: need this in order to deal with aluminum container
	}
	//	if (i==nLayers-1) addOuterSurf("Main",Out); // kbat: comment this out
  ...
  */
  {  
	...
	// kbat: Gap between target and premoderator
	//	FixedComp::setConnect(2,Origin-Z*(height[nLayers-1]/2.0+gapToTarget),-Z);
	//	FixedComp::setLinkSurf(2,-SMap.realSurf(SI+8));
  ...
  */
  {
    ...
    /*    const std::string RSurfPlus=(rIndex<0) ? Target.getLinkComplement(static_cast<size_t>(-(rIndex+1))) : Target.getLinkString(static_cast<size_t>(rIndex));
    const std::string RSurfMinus=(-rIndex<0) ? Target.getLinkComplement(static_cast<size_t>(-(-rIndex+1))) : Target.getLinkString(static_cast<size_t>(-rIndex));

    std::cout << "rIndex: " << rIndex << std::endl;
    std::cout << "top/bottom target planes: " << RSurfPlus << " " << RSurfMinus << std::endl;
    */
    createObjects(System,CModPtr, Target, rIndex);
    createLinks();
  ...
  void CylPreMod::addToInsertChain(attachSystem::ContainedComp& CC) const
  {
